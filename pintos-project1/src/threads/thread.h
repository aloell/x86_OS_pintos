...
struct thread
  {
    ...
    int32_t recent_cpu;			//+will be set to 0 in default
    int32_t nice;			//+will be set to 0 in default
    //every list element is a different lock waiter which has boosted this thread
    int origin_priority_list[7];		//+
    int origin_priority_index;
    //struct list origin_list;
    //struct origin_content oc;
    int64_t sleep_time;                     /* + */
    struct list_elem allelem;           /* List element for all threads list. */
    struct list_elem sleep_elem;           /* +List element for sleeped threads */
    /* Shared between thread.c and synch.c. */
    struct list_elem elem;              /* List element. */
    struct thread* donatee;		/*+thread which is priority donated */
#ifdef USERPROG
