...
fsutil_cat (char **argv)
{
  ...
  char abs_path[LONGEST_PATH_LENGTH];
  convert_to_abspath(abs_path,file_name);
  bool is_file=false;
  struct inode* inode=NULL;
  bool status=filesys_open (abs_path,&is_file,&inode);
  if(!status||!is_file)
	PANIC ("%s: open failed", abs_path);
  ...
  for (;;)
    {
	  ...
	  convert_to_abspath(abs_path,file_name);
          ...
          if (!filesys_create (abs_path, size,true))
            PANIC ("%s: create failed", abs_path);
  	  ...
  	  if(!status||!is_file)
		PANIC ("%s: open failed,status:%d, is_file:%d", abs_path,status,is_file);
  	  dst = file_open(inode);
