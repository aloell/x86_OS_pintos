              ...
              int archive_fd, bool *write_error) 
{
  ...
  if (file_fd >= 0) 
    {
      bool success;
      int id_file_inode=inumber (file_fd);
      int id_archive_inode=inumber (archive_fd);
      
      if (id_file_inode != id_archive_inode) 
        {
      ...
      close (file_fd);
      //printf("end of archive file suc!!!!,%s\n",file_name);
      return success;
    }
  else
    {
